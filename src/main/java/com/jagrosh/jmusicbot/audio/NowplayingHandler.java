...
public class NowplayingHandler
{
    ...
    private void updateAll()
    {
        ...
        for(long guildId: lastNP.keySet())
        {
            ...
            MessageCreateData msgData = handler.getNowPlaying(bot.getJDA());
            if(msgData==null)
            {
                msgData = handler.getNoMusicPlaying(bot.getJDA());
                toRemove.add(guildId);
            }
            try 
            {
                // TODO: understand what this is doing
                tc.editMessageById(pair.getValue(), MessageEditData.fromCreateData(msgData)).queue(m->{}, t -> lastNP.remove(guildId));
            } 
            catch(Exception e) 
            {
                toRemove.add(guildId);
